...
export class Convertor {
    ...
    public create(): Project {
        ...
        for (const label of this.labels) {
            if (label.type === "Label") {
                ...
                if (
                    this.options.warnOnEmptyLabels &&
                    !this.labelConversions.has(label.value[0])
                ) {
                    this.logger.warn(
                        `Label: ${
                            label.value[0]
                        } is unused! Please consider removing this label`,
                    );
                }
            }
        }
    ...
    private insertFunctions() {
        const stat = this.labels.find((a) =>
            a.type === "Label" && a.value[0] === "stat"
        );
        if (stat === undefined) {
            this.logger.error(
                "fatal: no stat label, shouldn't be possible due to order of events",
            );
            throw new Error();
        }
        ...
        for (const node of nodes) {
            if (node.type === "InsertDef") {
                ...
                const argumentIds = JSON.stringify(
                    fnData.args.entries().map(([i, _]) =>
                        MD5(`${func}:${i + 1}`)
                    )
                        .toArray(),
                );
                const argumentNames = JSON.stringify(
                    fnData.args.entries()
                        .map(([i, _]) => `${func}:${i + 1}`)
                        .toArray(),
                );
                const argumentDefaults = JSON.stringify(
                    fnData.args
                        .map((t) => t === "Any" ? "" : "false"),
                );
                ...
                for (const sprite of sprites) {
                    ...
                    {
                        const spr = this.project.targets.find((v) =>
                            v.name === spr_name.name
                        )!;
                    ...
                    {
                        const l = (<any> this.labels.find((a) =>
                            a.type === "Label" && a.value[0] === label
                        ))!.value[1];
                    ...
                    if (labelNodes === undefined) {
                        throw new Error("presumbably unreachalbe");
                    }
    ...
    private insertEvents() {
        ...
        if (stat?.type !== "Label") {
            throw new Error(
                "No stat label, this should be impossible due to order of events",
            );
        }
        for (const node of stat.value[1]) {
            const add = (value: () => Block) => {
                const { target, label } = node as Extract<
                    IlNode,
                ...
                const sprite = this.project.targets.find((a) =>
                    a.name === spriteName
                )!;
                ...
                add(() =>
                    <Block> {
                        ...
                        fields: {
                            "BROADCAST_OPTION": [
                                name,
                                this.broadcasts.has(name)
                                    ? this.broadcasts.get(name)
                                    : (() => {
                                        this.logger.error(
                                            `No broadcast called: ${name}`,
                                        );
                                        throw new ConvertorError();
                                    })(),
                            ],
    ...
    private convertValue(
        ...
        switch (value.key) {
            ...
            case "String":
                return [1, [10, value.value.toString()]];
            case "Sound": {
                const { name } = value;
            ...
            case "Builtin": {
                ...
                const opcode = (() => {
                    switch (builtinType.key) {
                        case "Costume":
                            return "looks_costumenumbername";
                        case "XPosition":
            ...
            case "SensingOperation": {
                ...
                const a = (
                    ...
                    if (value.key === "String") {
                        ...
                        this.logger.error(
                            "Cannot use literal value in this location, please use a join operator",
                        );
                        this.logger.error(
                            "Only literal valid in this position is a string literal",
                        );
                    } else {
                        const f = {} as any;
                        f[key] = [
                            3,
                            calc2.toString(),
                            this.convertValue(blocks, value, spr)[1],
                        ];
                        blocks.set(calculation.toString(), {
                            opcode: opcodeMain,
                            inputs: {},
            ...
            case "Bool":
                return this.convertValue(blocks, {
                    key: "BinaryOperation",
                    oper: "Eq",
    ...
    private convertInstruction(
        ...
        switch (node.type) {
            ...
            case "If":
                {
                    ...
                    const hds = (() => {
                        if (labelNode?.type !== "Label") {
                            throw "should be unreachable, not label";
                        }
                        const [nodes, heads] = this.convertLabel(
                            labelNode.value[1],
                            labelNode.value[0],
                            spr,
                        );
            ...
            case "Run":
                {
                    ...
                    if (func.args.length !== argAmount) {
                        ...
                        this.logger.error(
                            `Expected ${argAmount} arguments, got ${func.args.length}`,
                        );
                        throw new ConvertorError();
                    }
                    const argumentIds = JSON.stringify(
                        func.args.entries().map(([i, _]) =>
                            MD5(`${id}:${i + 1}`)
                        )
                            .toArray(),
                    );
            ...
            case "PenChangeValue":
                {
                    const opcode = node.type === "PenChangeValue"
    ...
    private gatherValues() {
        const stat = this.labels.find((a) =>
            a.type === "Label" && a.value[0] === "stat"
        ) as Extract<IlNode, { type: "Label" }> | undefined;
        if (stat === undefined) {
        ...
        for (const node of field) {
            switch (node.type) {
                ...
                case "AddSprCostume":
                    {
                        const { id, format, file, anchorX, anchorY, name } =
                            node;
                        ...
                        const target = this.project.targets.find((a) =>
                            a.name === sprite.name
                        )!;
                        const fmt = format.toLowerCase();
                        const targetPath = path.join(
                            this.options.resourcesFolderPath,
                            file,
                        );
                        ...
                        const m = MD5(
                            Array.from(fr).map((a) => String.fromCharCode(a))
                                .join(""),
                        );
                ...
                case "AddSprSound":
                    {
                        ...
                        if (format === "WAV") {
                            const wdata = (<any> WavDecoder.decode).sync(
                                Buffer.from(fr),
                            );
                            ...
                            this.logger.error(
                                "currently unsupported because i(x5ilky) for the love of god cannot figure out how to parse a mp3 header in deno",
                            );
                            this.logger.error(
                                "when i find a library for this ill add support for mp3's",
                            );
                            this.logger.error(
                                "for now, just convert your mp3's to wav's with ffmpeg/audacity",
                            );
                            throw new ConvertorError();
                        }
                ...
                case "CreateVar":
                    {
                        if (
                            node.name.startsWith(FORBIDDEN_VARIABLE_NAME_PREFIX)
                        ) {
                            this.logger.warn(
                ...
                case "Def": {
                    ...
                    this.variables.push(
                        `${FORBIDDEN_VARIABLE_NAME_PREFIX}${id}`,
                    );
                }
            }
        }
