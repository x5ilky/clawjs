        ...
        switch (instr.toLowerCase()) {
            ...
            case "addcostume": {
                ...
                const type = ((type): FileFormat => {
                    switch (type) {
                        case "png": case "bitmap":
                            return "PNG";
                        case "svg": case "vector":
                            return "SVG";
                        default:
                            this.error("Invalid file format! Files have to be:");
            ...
            case "keypress": {
                ...
                if (key.length > 1) {
                    if (!(["space", "left arrow", "up arrow", "right arrow", "down arrow", "any"].includes(key))) {
            ...
            case "stop": {
                const type = ((s: string): StopType => {
                    switch (s) {
                        case "this": return "ThisScript";
                        case "all": return "All";
                        case "other": return "OtherScripts";
                        default: 
                            this.error("Invalid option for stop command!");
                            this.error("Valid ones are: `this`, `all`, and `other`");
                            Deno.exit(1);
                    }
                })(this.getIdentifier(chars));
                return {
            ...
            case "clonemyself": return {
                type: "CloneMyself"
            };
            case "whenclone": return {
                type: "WhenClone",
                target: this.getIdentifier(chars),
                label: this.getLabel(chars)
            };
            case "deleteclone": return {
                type: "DeleteClone"
            };

            case "movesteps": return {
                type: "Move",
            ...
            case "thinkfor": return {
                type: "ThinkFor",
                value: this.getValue(chars),
                secs: this.getValue(chars),
            }
            case "switchcostume": return {
                type: "SwitchCostume",
                value: this.getValue(chars)
            }
            case "nextcostume": return {
                type: "NextCostume",
            }
            case "switchbackdrop": return {
                type: "SwitchBackdrop",
                value: this.getValue(chars)
            }
            case "nextbackdrop": return {
                type: "NextBackdrop",
            }
            case "changesize": return {
                type: "ChangeSize",
                value: this.getValue(chars)
            }
            case "setsize": return {
            ...
            case "gotolayer": return {
                type: "GotoLayer",
                value: { front: true, back: false }[this.getIdentifier(chars)] ?? (() => { this.error("gotolayer only takes front or back"); Deno.exit(1)})()
            };

            case "changelayer": return {
            ...
            case "warp": {
                ...
                for (let i = 0; i < amt; i++) {
                    const type = this.getIdentifier(chars);
                    if (type === "bool" || type === "boolean") args.push("Boolean");
                    else if (type === "any") args.push("Any");
                    else {
                        this.logger.warn("`def` keyword only accepts `bool` or `any`, defaulting to any...");
                        args.push("Any")
                    }
                }
                return {
