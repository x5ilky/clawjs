...
export class Optimizer {
    ...
    optimize(): void {
        ...
        while (true) {
            for (let i = 0; i < this.options.repeatsPerPass; i++) {
                if (this.options.redundantOperations) {
                    this.redundantOperationPass();
                }
                if (this.options.variableReplacements) {
                    this.redundantVariablePass();
                }
                if (this.options.redundantControlFlow) {
                    this.redundantControlFlowPass();
                }
            }
            ...
            return this.cloneValue(v, {
                BinaryOperation: (v) => {
                    ...
                    if (v.oper === "Mul") {
                        ...
                        if (v.left.key === "Float" && v.left.value === 1) {
                            return v.right;
                        }
                        if (v.right.key === "Float" && v.right.value === 1) {
                            return v.left;
                        }
                        if (v.left.key === "Float" && v.left.value === 0) {
                            return v.left;
                        }
                        if (v.right.key === "Float" && v.right.value === 0) {
                            return v.right;
                        }
                    }
                    if (v.oper === "Div") {
    ...
    redundantVariablePass(): void {
        ...
        for (const [k, v] of redundantVars) {
            ...
            if (v.count === 0) {
                this.statLabel.splice(
                    this.statLabel.findIndex((a) =>
                        a.type === "CreateVar" && a.name === k
                    ),
                    1,
                );
            }
                ...
                if (node.type === "If") {
                    if (node.predicate.key === "Bool") {
                        if (node.predicate.value) {
                            const l = this.labels.find((a) =>
                                a.type === "Label" && a.value[0] === node.label
                            );
    ...
    countVariableAssignments(): Map<
        string,
        { count: number; definitions: IlValue[] }
    > {
        const map = new Map<
            string,
            { count: number; definitions: IlValue[] }
    ...
    replaceVariableUsageInValue(
        value: IlValue,
        map: Map<string, IlValue>,
    ): IlValue {
    ...
    cloneValue(
        value: IlValue,
        cb: { [k in IlValue["key"]]?: (v: IlValue) => IlValue },
    ): IlValue {
        switch (value.key) {
            case "Integer":
            case "String":
                        ...
                        case "ColorIsTouchingColor":
                            return c({
                                ...
                                oper: {
                                    ...
                                    color1: this.cloneValue(
                                        value.oper.color1,
                                        cb,
                                    ),
                                    color2: this.cloneValue(
                                        value.oper.color2,
                                        cb,
                                    ),
                                },
