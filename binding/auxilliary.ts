...
import type { IlNode, IlValue } from "../ir/types.ts";
import {
    abs,
    add,
    and,
    DataClass,
    type DataclassOutput,
    div,
    eq,
    gt,
    gte,
...
export class FixedList<
    T extends new () => Serializable & Variable,
    Size extends number,
> implements Serializable, Variable {
    type: T;
    size: Size;
    ...
    literal(...values: InstanceType<T>[]) {
        if (this.size !== values.length) {
            throw new Error(
                `literal function took different argument length compared to specified FixedArray size`,
            );
        }
        for (let i = 0; i < this.size; i++) {
 ...
 */
export const litMax: (a: Valuesque, b: Valuesque) => IlWrapper = (
    a: Valuesque,
    b: Valuesque,
) => div(add(add(a, b), abs(sub(a, b))), 2);
/**
 ...
 */
export const litMin: (a: Valuesque, b: Valuesque) => IlWrapper = (
    a: Valuesque,
    b: Valuesque,
) => div(sub(add(a, b), abs(sub(a, b))), 2);
...
export const sign: (x: Valuesque) => IlWrapper = (x: Valuesque) =>
    mul(not(eq(x, 0)), div(x, abs(x)));
/**
 ...
 */
export const pow: (base: Valuesque, exponent: Valuesque) => IlWrapper = (
    base: Valuesque,
    exponent: Valuesque,
) => (tenpower(mul(exponent, log(abs(base)))));
